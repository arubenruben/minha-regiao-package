...
From the repo root, with `extract_geo` synced (`uv sync --package extract_geo`):
...
uv run --package extract_geo python -m extract_geo.sub_flows.extract_cities.ExtractCities
uv run --package extract_geo python -m extract_geo.sub_flows.extract_districts.ExtractDistricts
```
