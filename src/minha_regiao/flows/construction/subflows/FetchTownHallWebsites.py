...
def validate_town_hall_header(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.columns = [" ".join(col.split()).lower() for col in df.columns]
    ...
    if missing_columns:
        logger.error(
            f"Spreadsheet header is missing required columns: {missing_columns}"
        )
...
def parse_town_hall_data(df: pd.DataFrame) -> Sequence[City]:
    ...
    for _, row in df.iterrows():
        ...
        cities.append(City(name=city_name, district=district, town_hall=town_hall))

    if len(cities) != 308:
        logger.error(
            f"Expected 308 cities, but found {len(cities)}. Please check the source file for missing or extra entries."
        )
